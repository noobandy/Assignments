...
public class SignUpFormActivity extends Activity {
	...
	public void submitSignUpForm(){
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(results, firstName, "First Name is required");
		ValidationUtils.rejectIfEmptyOrWhitespace(results, lastName, "Last Name is required");
		ValidationUtils.rejectIfEmptyOrWhitespace(results, email, "Email is required");
		ValidationUtils.rejectIfEmptyOrWhitespace(results, password, "Password is required");
		if(results.isValid()){
			String sex = null;
			if(male.isChecked()){
				sex = "Male";
			...
			SignUpForm signUpForm = new SignUpForm(firstName.getText().toString(), lastName.getText().toString(), email.getText().toString(), password.getText().toString(), confirmPassword.getText().toString(), sex, country.getSelectedItem().toString());

			Intent intent = new Intent(this, SignUpDetailsActivity.class);
			intent.putExtra("signUpForm", signUpForm);
